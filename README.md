...
- **Local or cloud transcription**: [Parakeet CoreML](https://github.com/badlogic/parakeet-coreml) on Apple Silicon, [Sherpa-ONNX Parakeet](https://k2-fsa.github.io/sherpa/onnx/) as the local path for Intel-based Macs where `parakeet-coreml` is not supported (and an alternative on Apple Silicon), or OpenAI Whisper in the cloud
...
2. Copy the example environment file and fill it in:
   ...
   - `SHERPA_ONNX_MODEL_DIR` *(optional)* — path to an extracted Sherpa-ONNX Parakeet model directory (primarily for Intel-based Macs, where `parakeet-coreml` is not supported)
...
2. **Sherpa-ONNX Parakeet** — the local/offline path added for Intel-based Macs, where `parakeet-coreml` does not run (and a CPU fallback on Apple Silicon)
...
### Installing Sherpa-ONNX Parakeet (local transcription for Intel-based Macs)

This is the recommended local transcription path on Intel-based Macs, since `parakeet-coreml` is Apple-Silicon-only. It can also be used on Apple Silicon, but TelePi will still prefer Parakeet CoreML there when available.
